...
def write_array_to_file(np_array, labels, partition, dst_dir):
    ...
    for i in range(len(np_array)):
        ...
        filename = partition + "_" + str(i) + ".jpg"
        path = os.path.join(dst_dir, str(label), filename)
        image.save(path)
...
def main(save_dir):
    ...
    img_train, lbls_train = train_images(), train_labels()
    img_test, lbls_test = test_images(), test_labels()
    write_array_to_file(img_train, lbls_train, "train", save_dir)
    write_array_to_file(img_test, lbls_test, "test", save_dir)
